...
module Validator
  ...
  ACTIONS = %w[recommends accepts].freeze

  def validate(data)
    validated_data = data.map.with_index do |line, index|
      validate_line(line.strip.split(' '), index)
    end
    validated_data.compact.sort { |a, b| a[:time] <=> b[:time] }
  end
  ...
  def validate_action(line, index)
    ...
    add_error(index, 'Inviter and Invitee cannot be same') if action == ACTIONS[0] && line[2] == line[4]
    action
  end
